...
lib.addCommand('setcryptoworth', {
    ...
    if not crypto then
        exports.qbx_core:Notify(src, Lang:t('text.you_have_not_provided_crypto_available_eternal'))
        return
    end
...
lib.addCommand('checkcryptoworth', nil, function(source)
    exports.qbx_core:Notify(source, Lang:t('text.the_eternal_has_a_value_of', {
...
lib.callback.register('qb-crypto:server:BuyCrypto', function(source, data)
    ...
    if Player and Player.PlayerData.money.bank >= total_price then
        ...
        TriggerClientEvent('qb-phone:client:AddTransaction', source, Player, data, Lang:t('credit.you_have_eternal_purchased', { dataCoins = tonumber(data.Coins) }), "Credit")
...
lib.callback.register('qb-crypto:server:TransferCrypto', function(source, data)
    ...
    if Player and Player.PlayerData.money.crypto >= tonumber(data.Coins) then
        ...
        Player.Functions.RemoveMoney('crypto', tonumber(data.Coins))
        TriggerClientEvent('qb-phone:client:AddTransaction', source, Player, data, "You have " .. tonumber(data.Coins) .. " Eternal('s) transferred!", "Depreciation")
        ...
        if Target then
            ...
            TriggerClientEvent('qb-phone:client:AddTransaction', Target.PlayerData.source, Player, data, "There are " .. tonumber(data.Coins) .. " Eternal('s) credited!", "Credit")
        else
